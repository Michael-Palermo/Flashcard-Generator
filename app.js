...
function createCard() {
  ...
  .prompt([
    ...
    if (response.cardType === choices[0]) {
      basicCard();
    }
    else {
      clozeCard();
...
function clozeCard() {
  ...
  .prompt([
    ...
    console.log('-----------------NEW CARD AVAILABLE------------------------')
    ...
    if (cardCounter < basicCards.length) {
      ...
      .prompt([
        ...
        if (response.userInput === basicCards[cardCounter].back) {
          ...
          console.log('Nope. The correct answer is ' + basicCards[cardCounter].back + '.');
          cardCounter++;
          testBasic();
        }
      })
    ...
    if (allCards[cardCounter].front){
      ...
      .prompt([
        ...
        if (response.userInput === allCards[cardCounter].back) {
          ...
          console.log('Nope. The correct answer is ' + allCards[cardCounter].back + '.');
          cardCounter++;
          testAll();
        }
      })
